...
class RolloutBuffer:
    ...
    def getReward(self, state):
        ...
        # DRIFT PENALTY
        root_vel = body[:, ROOT, [7, 9]]
        drift_penalty = (
            0.1 * root_vel.pow(2).sum(-1)
            + 
            0.1 * root_vel.abs().sum(-1)
        )
        self.drift_logs.append(drift_penalty.mean().item())
        self.vel_logs.append(root_vel.mean(dim=0).detach().cpu())
        ...
        return 0.65 * r_pose + 0.15 * r_end + 0.1 * r_com - drift_penalty
...
for iteration in range(start_iteration, N):
    ...
    avg_drift = np.mean(buffer.drift_logs)
    avg_vel = torch.stack(buffer.vel_logs).mean(dim=0)

    print(
        f"[drift] vel=({avg_vel[0]:+.4f}, {avg_vel[1]:+.4f}) "
        f"penalty={avg_drift:.4f}"
    )
