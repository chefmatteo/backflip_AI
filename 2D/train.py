...
class RolloutBuffer:
    ...
    def getReward(self, state):
        ...
        stepping = 0.05 * (state[:, 37] - state[:, 38]).abs()
        return 0.2 + upright + height + stepping
...
def updateModel(impulse_mag):
    ...
    for epoch in range(K_epochs):
        ...
        for start in range(0, N_samples, batch_size):
            ...
            # dual-clip: when advantage is negative, a huge ratio makes the surrogate
...
for iteration in range(start_iteration, N):
    ...
    # curriculum: advance impulse only after sustained performance over a window, with
